...
export default class Activity10 extends Component {
  ...
  db.transaction((tx) => {
      console.log("mode="+this.props.wrong);
       tx.executeSql('SELECT * FROM act10 WHERE chapter=? AND topic=? AND status=?', [chapter,topic,this.props.wrong], (tx, results) => {
       ...
       if(len > 0){
        ...
        this.setState({answer: row.question});
        this.setState({correct_ans: row.answer});
        ops.push(row.option_a);
        ops.push(row.option_b);
        ops.push(row.option_c);
  ...
  _handleSubmitPress = () => {
    ...
    if(this.state.current_ans === this.state.correct_ans){
      ...
      db.transaction((tx) => {
        tx.executeSql('UPDATE act10 SET status=1 WHERE id=?', [this.state.id], (tx, results) => {
      ...
      db.transaction((tx) => {
        tx.executeSql('UPDATE act10 SET status=2 WHERE id=?', [this.state.id], (tx, results) => {
